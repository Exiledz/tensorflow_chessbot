...
def trimBoard(img_arr_gray):
  maxLoc = 0
  maxDiff = 0
  #Find the x position of the row with the bottom line of the board
  for x in range(int(len(img_arr_gray)/2), len(img_arr_gray) - 1):
    diff = np.abs(img_arr_gray[x+1].sum()-img_arr_gray[x].sum())
    if(diff > maxDiff):
      maxDiff = diff
      maxLoc = x

  return img_arr_gray[:maxLoc+1]

def findChessboardCorners(img_arr_gray, noise_threshold = 1000):
  ...
  while(trimCount < 10):
    ...
    if len(seqs_x) == 0 or len(seqs_y) == 0:
      #Could not determine board, usually caused by noise, trim board until found or give up
      img_arr_gray = trimBoard(img_arr_gray)
      continue
